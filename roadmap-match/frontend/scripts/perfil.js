...
// Guardar la imagen actual al cargar la página

/* --- MOSTRAR VISTA PREVIA DE LA IMAGEN --- */
inputImagen.addEventListener("change", (e) => {
  let imagenActual = previewImagen.src;
  const file = e.target.files[0];
  if (file) {
    if (file.size > 1 * 1024 * 1024) {
      mensaje.textContent = "La imagen no puede superar 1 MB";
      mensaje.style.color = "red";
      inputImagen.value = ""; // limpiar input para que no se suba
      previewImagen.src = imagenActual; // volver a la imagen que ya tenía el usuario
      return;
    }
...
async function cargarDatosUsuario() {
  try {
    ...
    mensaje.textContent = "Error al traer datos del usuario";
    mensaje.style.color = "red";
  }
}
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // Mostrar error de multer por tamaño de imagen u otro error
